...
const WA = "https://wa.me/529612513578?text=" + encodeURIComponent("Hola, me interesa cotizar productos Voltena / Nabota.");
...
const badges = ["MFDS", "FDA", "CE", "🇰🇷 Made in Korea", "Dist. Autorizado"];
...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      {/* Gradient overlay — fades 3D into black at bottom */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 md:px-12 pt-28 pb-20 flex flex-col items-center text-center">
        ...
        <h1
          ...
          style={{ fontSize: "clamp(2.6rem, 6vw, 6rem)", fontWeight: 300, lineHeight: 1.1, letterSpacing: "-0.02em" }}
        >
          <BlurText
            text="Dermocosmética Coreana de Élite en México"
            delay={80}
          />
        </h1>
        ...
        >
          ...
          <span style={{ color: "#c4a96a" }}>Voltena</span> y{" "}
          <span style={{ color: "#c4a96a" }}>Nabota</span> para médicos estéticos,
        ...
        <div ref={ctaRef} className="flex flex-col sm:flex-row gap-3 mb-14">
          <a
            ...
            className="lg inline-flex items-center justify-center gap-2 px-8 py-3 rounded-full font-body text-sm tracking-widest uppercase font-medium transition-all duration-300"
            style={{ color: "#00d4e8", border: "1px solid rgba(0,212,232,0.4)" }}
          ...
          <a
            href="#products"
            className="inline-flex items-center justify-center gap-2 px-8 py-3 rounded-full font-body text-sm tracking-widest uppercase font-medium text-white/60 hover:text-white transition-all duration-300 border border-white/10 hover:border-white/20"
          >
            Ver Catálogo
          </a>
        </div>
        ...
        {/* Stats row */}
        <div ref={statsRef} className="flex flex-wrap justify-center gap-px mb-12 rounded-full overflow-hidden lgs">
          {stats.map((s, i) => (
            <div key={i} className="px-6 py-4 text-center">
              <p className="font-heading text-white italic" style={{ fontSize: "1.6rem", fontWeight: 300, lineHeight: 1 }}>
                {s.value}
              </p>
              <p className="font-body text-white/35 mt-1" style={{ fontSize: "10px", letterSpacing: "0.15em", textTransform: "uppercase" }}>
                {s.label}
              </p>
            </div>
          ))}
        ...
        <div ref={badgesRef} className="flex flex-wrap justify-center gap-2">
          {badges.map((b, i) => (
            <span
              key={i}
              className="font-body px-3 py-1 rounded-full text-white/30 border border-white/[0.08]"
